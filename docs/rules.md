...
4. **Terminal Halt on Non-Recoverable Errors:** Expired tokens, exceeded price ceilings, and signature mismatches are treated as terminal failures, not retryable errors — retrying will not change a deterministic outcome, so the loop must halt immediately rather than burn retry budget.
